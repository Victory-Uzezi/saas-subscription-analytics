...
for uid, signup in zip(user_ids, signup_dates):
    ...
    last_active_date = (
        churn_date - timedelta(days=random.randint(1, 30)) if churned else END_DATE
    )
    ...
    for date in all_dates:
        activity_data.append({
            ...
            'features_used': np.random.choice(
                ['Task Tracking', 'Collaboration', 'Reporting', 'Automations']
            ),
            'subscription_status': np.random.choice(
                ['Active', 'Cancelled', 'Trial'], p=[0.7, 0.2, 0.1]
            )
        })
...
monthly_kpis_df.to_csv(os.path.join(output_dir, "monthly_kpis.csv"), index=False)
daily_activity_df.to_csv(os.path.join(output_dir, "daily_activity.csv"), index=False)
